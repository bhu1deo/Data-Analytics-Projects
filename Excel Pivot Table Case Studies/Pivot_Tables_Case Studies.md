...
1. **The figures are erroneous :** The revenue reported by a particular film is only US based. For example Pirates of the Caribbean 3 grossed $309M in the US, and another $650M worldwide. But in the raw data, only the first figure has been recorded. 
1. **Significance of the currency denomination is NOT factored in.** For example the Japanese movie Godzilla has it’s budget replicated from Yen to Dollars (Conversion not carried out). Hence better to stick with USA based films. 

Questions:

1. **Limit yourself to the USA films. And 21st Century Films.** Add a date timeline and Filter every Pivot table independently. 
1. Compute the metric : ROI and compare it with buckets of IMDB score. Also compare it with Rating associated with these movies. Can you generalize?
1. Find out ROI by budget buckets. Can you find a pattern? Find ROI by Genre and FB likes. 
1. Over the Years 1980-2015 : Find the number of Movies which could not reach break-even. Does the Trend suggest anything? 

Findings : 

1. **ROI vs IMDB Score :**  As expected higher IMDB scores mean higher ROI (on an average). Films which are critically acclaimed also tend to be commercial  successes. Further more most films are rated “Average” by viewers. The “Average” bucket is 6-7.5 IMDB score. So, producers and directors should strive to make “Good Films”, else they risk losing money. 
1. **ROI vs Rating :** Most titles belong to R and PG-13 rated films. This is followed by G rated films. Films made for the General Public comprise of a paltry share (2% of the total). As far as ROI is concerned G rated films on average do substantially well than the other 3, which have a similar ROI of 1.3.
1. **ROI vs Budget :** Again, we see a clear trend here. As the budget of a movie increases, on an Average the ROI decreases. So the aim should be to make a good movie in a manageable budget. Since, higher budgets also present higher risk.
1. ` `**BreakEven :** Note that the Revenue generated by a particular film is limited to USA gross. Worlwide collection is NOT taken into account in this dataset. We observed that since 1990s, many more films lost their money. Almost half of the films couldn’t recover their budget. This trend was noticed in the 2010s as well. 
1. **ROI vs Genre :** Documentary, Horror and Fantasy doubled your money on an Average. Whereas Crime, Drama and Action were just above BreakEven. Again note that the revenues generated are US based only. 

**US Voters 2012 Dataset**

Questions:

1. Firstly, find (irrespective of age group), the % of people not residing as citizens. 
1. In different table layouts visualize the % of voters w.r.t both state and age. State field outer than age field. Can we answer questions like : Are voters mostly Young or Old? 

**San Francisco Salaries** 

Questions:

1. Group Job titles by related fields, for example there might be n different job titles pertaining to an Accountant. 
1. Find out the Average base pays alongwith the Number of people engaged in that particular job. Do the same for Overtime pay as well. 

**Shark Attacks**

Questions:

1. Yearly Number of Shark attacks. Also attacks by location. Number of attacks by Type. Fatality Analysis. Group similar fields like Boat and Boating. 
